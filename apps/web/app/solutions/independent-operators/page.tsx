...
const SLUG = '/solutions/independent-operators'

export async function generateMetadata(): Promise<Metadata> {
  const t = getDict(getLocale()).solutionPages.independentOperators
...
export default function IndependentOperatorsSolutionPage() {
  const locale = getLocale()
  const dict = getDict(locale)
  const content = dict.solutionPages.independentOperators
  const baseUrl = getAppUrl()
